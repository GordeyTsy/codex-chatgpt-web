...
test("two-part saved chats re-prove unchanged effort after the first message creates the conversation URL", async () => {
  ...
  const page = Object.assign(new EventEmitter(), {
    ...
    evaluate: async () => true, // Autolink compatibility is already initialized in this fixture.
  });
  let sends = 0;
...
test("a stage that spans a system sleep is not charged for the slept time", async () => {
 ...
 test("unavailable requested model is actionable and never retried or silently downgraded", () => {
  const error = chatGptRequestedModeUnavailableError("Pro");
  expect(error.message).toContain("requested Pro mode");
  expect(error.message).toContain("selected model was not changed");
  expect(error.message).not.toContain("usage limit");
  expect(error.code).toBe("chatgpt_model_unavailable");
