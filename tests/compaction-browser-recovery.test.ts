...
import { ChatGptBrowserWorker, chatGptRequestedModeUnavailableError } from "../src/adapters/chatgpt-web/browser-worker";
...
test.each([[true, false, true], [false, false, true], [true, true, true], [true, false, false]])("browser turns preserve recovery, ordering and final-only tools (owned=%s, tools=%s, multipart=%s)", async (owned, tools, multipart) => {
  ...
  try {
    ...
    expect(actions).toEqual([
      ...(multipart ? [
        `effort:${effort}`, // Final mode is checked before the first staging message.
        "effort:low",
...
test("exhausted requested Pro quota prevents all multipart staging sends", async () => {
  const diagnostics = mkdtempSync(join(tmpdir(), "quota-preflight-"));
  const unavailable = chatGptRequestedModeUnavailableError("Pro");
  const actions: string[] = [];
  let released = false;
  const page = Object.assign(new EventEmitter(), {
    evaluate: async () => true, // This retained document already has compatibility installed.
    isClosed: () => false,
  });
  const worker = Object.assign(Object.create(ChatGptBrowserWorker.prototype), {
    config: { appName: "Codex Native2", browserDiagnosticsPath: diagnostics },
    runStage: async (_trace: string, _name: string, _timeout: number, action: (signal: AbortSignal) => Promise<unknown>) => action(new AbortController().signal),
    prepareChatSurface: async () => {},
    selectModelAndEffort: async (_page: unknown, _model: string, effort: string) => {
      actions.push(`effort:${effort}`);
      throw unavailable;
    },
    attachPrompt: async () => { actions.push("attach"); },
    sendAttachedPrompt: async () => { actions.push("send"); },
  });
  try {
    await expect(worker.runBrowserTurn({
      traceId: "quota_preflight_fixture", modelId: "gpt-5.6-sol", reasoning: "max",
      capabilities: { localToolsEnabled: false, solAvailable: true, extraHighAvailable: true, proAvailable: true },
      prepare: async () => ({ text: "Synthetic context", images: [],
        multipart: { parts: Array.from({ length: 6 }, (_, index) => JSON.stringify({ part: index + 1 })), commit: "Summarize" },
        release: () => { released = true; },
      }),
    }, undefined, page)).rejects.toBe(unavailable);
    expect(actions).toEqual(["effort:max"]);
    expect(released).toBe(true);
    expect(page.listenerCount("request")).toBe(0);
    expect(page.listenerCount("response")).toBe(0);
  } finally {
